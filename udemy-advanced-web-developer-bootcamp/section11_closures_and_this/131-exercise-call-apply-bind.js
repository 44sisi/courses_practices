...
function sumEvenArguments() {
  ...
  return newArgs.reduce((acc, next) => {
    if (next % 2 === 0) {
      return acc + next;
    } else {
      return acc;
...
function invokeMax(fn, num) {
  let max = 0;
  return function () {
    // have to be function, can't use big arrow notaiton because it will not change the context
    if (max >= num) {
      return 'Maxed Out!';
    } else {
      max++;
...
Flip should return a new function that when invoked, will invoke the function passed to flip 
    with the correct value of the keyword this and all of the arguments passed to the function REVERSED. 
HINT 
    - if you pass more than two parameters to flip, those parameters should be included as parameters to the inner function when it is invoked. 
    You will have to make use of closure! 
...
function flip(fn, thisArg) {
  ...
  return function () {
    ...
    return fn.apply(
      thisArg,
      [...outerArgs, ...innerArgs].slice(0, fn.length).reverse()
    );
  };
}
